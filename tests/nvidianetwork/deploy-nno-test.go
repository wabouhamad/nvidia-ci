...
var (
	...
	ofedDriverVersionArm64 = "24.10-0.5.5.0-0"
	ofedRepository         = map[string]string{
		"amd64": "nvcr.io/nvidia/mellanox",
	...
	nnoCatalogSource                         = "undefined"
	nnoSubscriptionChannel                   = "undefined"
	nnoInstallPlanApproval v1alpha1.Approval = "Automatic"

	nnoDefaultSubscriptionChannel        = "undefined"
	networkOperatorUpgradeToChannel      = "undefined"
	...
	Context("DeployNNO", Label("deploy-nno-with-dtk"), func() {
		...
		It("Deploy NVIDIA Network Operator with DTK", Label("nno"), func() {
			...
			if deployFromBundle {
				...
				if nnoPkgManifestBuilderByCatalog == nil {
					...
					if createNNOCustomCatalogsource {
						...
						By("Sleep for 30 seconds to allow the NNO custom catalogsource to be created")
						time.Sleep(30 * time.Second)
			...
			if deployFromBundle {
				...
				if nnoSubscriptionChannel != "undefined" {
			...
			// For arm64 servers, we don't have all the ofed driver images yet
			// Temporary workarounds for arm64 servers
			By("Updating NicClusterPolicyBuilder object driver version and driver repository for arm64 clusters")
			glog.V(networkparams.LogLevel).Infof("Updating NicClusterPolicyBuilder object driver version and " +
				"driver repository for arm64 clusters")
			nicClusterPolicyBuilder.Definition.Spec.OFEDDriver.Repository = ofedRepository[clusterArch]

			if clusterArch == "arm64" {
				glog.V(networkparams.LogLevel).Infof("Setting the OFED driver for arm64 system to '%s'",
					ofedDriverVersionArm64)
				nicClusterPolicyBuilder.Definition.Spec.OFEDDriver.Version = ofedDriverVersionArm64
			}
