...
func TestDRANative(t *testing.T) {
	_, reporterConfig := GinkgoConfiguration()
	reporterConfig.JUnitReport = inittools.GeneralConfig.GetJunitReportPath(currentFile)

	RegisterFailHandler(Fail)
	RunSpecs(t, "DRA Native", Label(tsparams.DRANativeLabels...), reporterConfig)
}

var _ = JustAfterEach(func() {
	specReport := CurrentSpecReport()
	reporter.ReportIfFailed(
		specReport, currentFile, tsparams.DRANativeReporterNamespacesToDump, tsparams.DRANativeReporterCRDsToDump,
		clients.SetScheme)
})
...
var _ = AfterSuite(func() {
	...
	if scriptPath != "" {
		artifactDir := inittools.GeneralConfig.GetReportPath("dra-native-tests-must-gather")
